    ...
    <div style={{
      ...
      <div style={{ padding: "16px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
          ...
          <span style={{ fontSize: 12, padding: "2px 7px", borderRadius: 9999, color: p.color, background: `${p.color}15`, fontWeight: 600 }}>
            {p.overtimeRate}% 超时率
          </span>
        </div>
        ...
        <div style={{ marginBottom: 10 }}>
          <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", marginBottom: 4 }}>预估处罚金额</p>
          <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
            <span className="mono" style={{ fontSize: 30, fontWeight: 800, color: p.color, letterSpacing: "-0.03em" }}>
              ¥{p.amount.toLocaleString()}
            </span>
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: isUp ? "var(--danger)" : "var(--success)" }}>
                {isUp ? "▲" : "▼"} {Math.abs(p.change)}%
              </div>
              <div style={{ fontSize: 12, color: "var(--text-faint)" }}>同比上月</div>
            </div>
          </div>
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "var(--text-muted)" }}>
            涉及 <span style={{ color: "var(--text-secondary)", fontWeight: 600 }}>{p.orders} 单</span>
          </p>
        </div>

        {/* Rule */}
        <div style={{ padding: "8px 10px", background: "var(--bg)", borderRadius: 6, marginBottom: 12 }}>
          <p style={{ margin: 0, fontSize: 12, color: "var(--text-faint)" }}>处罚规则：{p.rule}</p>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {p.breakdown.map((b) => (
            <div key={b.label} style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <span className="mono" style={{ fontSize: 13, color: "var(--text-muted)" }}>{b.val}</span>
            </div>
          ))}
        </div>
      </div>
